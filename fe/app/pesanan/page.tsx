...
function OrderCard({ order }: { order: Order }) {
  ...
  return (
    <div
      style={{ border: "1px solid rgba(200,183,158,0.1)", background: "rgba(252,250,246,0.02)", marginBottom: 10, transition: "border-color 0.25s ease" }}
      onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(200,183,158,0.22)"; }}
      onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(200,183,158,0.1)"; }}
    >
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", flexShrink: 0 }}>
            {order.items.slice(0, 3).map((item, i) => {
              ...
              return (
                <div key={item.id} style={{ width: 40, height: 50, position: "relative", overflow: "hidden", background: "var(--color-surface)", border: "1px solid rgba(200,183,158,0.08)", marginLeft: i > 0 ? -10 : 0, zIndex: 3 - i }}>
                  {img && <Image src={img} alt={item.product.name} fill sizes="44px" className="object-cover" style={{ filter: "brightness(0.85)" }} />}
                </div>
              );
            })}
            {order.items.length > 3 && (
              <div style={{ width: 40, height: 50, background: "rgba(200,183,158,0.1)", border: "1px solid rgba(200,183,158,0.08)", marginLeft: -10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "rgba(200,183,158,0.6)", zIndex: 0 }}>
                +{order.items.length - 3}
              </div>
            )}
          </div>
          ...
          <div style={{ minWidth: 0, textAlign: "left" }}>
            <div style={{ fontFamily: "var(--font-serif)", fontSize: 14, color: "var(--color-ink)", marginBottom: 4 }}>{order.order_number}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontFamily: "var(--font-serif)", fontSize: 14, color: "var(--color-ink)" }}>{rupiah(order.total_amount)}</div>
          </div>
          <svg width="11" height="11" viewBox="0 0 11 11" fill="none" stroke="rgba(200,183,158,0.75)" strokeWidth="1.3"
      ...
      {/* expanded detail */}
      {open && (
        <div style={{ borderTop: "1px solid rgba(200,183,158,0.07)", padding: "16px 20px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 16 }}>
            {order.items.map((item) => {
              const img = parseImage(item.product.image_url);
              return (
                <div key={item.id} style={{ display: "flex", gap: 14, alignItems: "center" }}>
                  <div style={{ width: 52, height: 65, flexShrink: 0, position: "relative", overflow: "hidden", background: "var(--color-surface)" }}>
                    {img && <Image src={img} alt={item.product.name} fill sizes="56px" className="object-cover" style={{ filter: "brightness(0.88)" }} />}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 7, letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(200,183,158,0.78)", marginBottom: 3 }}>{item.product.code}</div>
                    <div style={{ fontSize: 13, color: "var(--color-ink)", lineHeight: 1.3, marginBottom: 5 }}>{item.product.name}</div>
                    {(item.product_variant.size || item.product_variant.color) && (
                      <div style={{ display: "flex", gap: 5 }}>
                        {[item.product_variant.size, item.product_variant.color].filter(Boolean).map((v) => (
                          <span key={v} style={{ fontSize: 8, letterSpacing: "0.14em", textTransform: "uppercase", color: "color-mix(in srgb, var(--color-ink) 78%, transparent)", border: "1px solid rgba(200,183,158,0.2)", padding: "2px 8px" }}>{v}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: 9, color: "color-mix(in srgb, var(--color-ink) 65%, transparent)", marginTop: 2 }}>×{item.quantity}</div>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ borderTop: "1px solid rgba(200,183,158,0.07)", paddingTop: 12, display: "flex", flexDirection: "column", gap: 5 }}>
            ...
            <div style={{ marginTop: 6, fontSize: 10, color: "color-mix(in srgb, var(--color-ink) 65%, transparent)", display: "flex", gap: 10 }}>
              ...
              <span style={{ color: "rgba(200,183,158,0.2)" }}>·</span>
      ...
      <main className="min-h-screen pt-28 pb-24" style={{ background: "var(--color-bg)" }}>
        <div className="mx-auto max-w-2xl px-6">
          ...
          <div className="mb-10">
            ...
            <div style={{ marginTop: 14, height: 1, maxWidth: 180, background: "linear-gradient(to right, rgba(200,183,158,0.4), transparent)" }} />
          </div>
          ...
          {status !== "loading" && !session && (
            <div style={{ textAlign: "center", paddingTop: 56, paddingBottom: 56 }}>
              ...
              <button
                ...
                style={{ fontSize: 9, letterSpacing: "0.28em", textTransform: "uppercase", color: "var(--color-accent)", border: "1px solid rgba(200,183,158,0.32)", padding: "13px 32px", background: "transparent", cursor: "pointer", transition: "all 0.2s ease" }}
          ...
          {(status === "loading" || (session && loading)) && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingTop: 8 }}>
              {[1, 2, 3].map((i) => (
                <div key={i} style={{ height: 82, background: "rgba(252,250,246,0.03)", border: "1px solid rgba(200,183,158,0.06)", animation: "pulse 1.6s ease infinite" }} />
              ))}
            </div>
          )}
          ...
          {session && !loading && (
            <>
              {/* tabs */}
              <div style={{ display: "flex", borderBottom: "1px solid rgba(200,183,158,0.08)", marginBottom: 20 }}>
                {TABS.map(({ key, label }) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        borderBottom: active ? "1px solid rgba(200,183,158,0.75)" : "1px solid transparent",
                        marginBottom: -1,
                        cursor: "pointer",
                        fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase",
                        color: active ? "var(--color-accent)" : "color-mix(in srgb, var(--color-ink) 75%, transparent)",
                    ...
                    >
                      {label}
                      {count > 0 && (
                        <span style={{ fontSize: 8, padding: "1px 6px", borderRadius: 99, background: active ? "rgba(200,183,158,0.25)" : "rgba(200,183,158,0.07)", color: active ? "var(--color-accent)" : "color-mix(in srgb, var(--color-ink) 65%, transparent)" }}>
                          {count}
                        </span>
                      )}
                    </button>
